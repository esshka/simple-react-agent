#!/usr/bin/env python3
"""Deep research demo using DuckDuckGo search and simple page fetch.
...
def make_web_search_tool(default_region: str, default_time: str | None, default_max: int) -> ToolSpec:
    """DuckDuckGo search (via duckduckgo_search). Returns compact results."""
    ...
    return ToolSpec(name="web_search", description="Search the web via DuckDuckGo and return top results (title, url, snippet)", parameters=params, handler=handler)
...
def main(argv: List[str]) -> int:
    ...
    agent = ReActAgent(
        ...
        system_prompt=(
            ...
            "then use fetch_page to extract key passages. Plan briefly as bullet points, avoid hallucinations, "
            "and cross-check claims. Provide a structured Final Answer with: Key Findings, Evidence with inline citations "
            "[1], [2], Counterpoints, Gaps/Limitations, and a Sources list with titles and URLs."
        ),
        keep_history=True,
        temperature=0.1,
        max_rounds=8,
